...
class MY_Controller extends CI_Controller
{
	...
	{
		if (empty($userdata['role'])) $userdata['role'] = 0;
	...
	public function initComm()
	{
		...
		else
		{
			...
			foreach ($_retdata as $k => $v) $retdata[$v['confkey']] = $v['confval'];
		...
		else
		{
			...
			foreach ($dddd as $k => $v)
			{
				if ($v['parentid'] != 0)
				{
					if (!empty($nativedata[$v['parentid']])) $nativedata[$v['parentid']]['subcate'][$k] = $dddd[$k]; 
				}
			}
