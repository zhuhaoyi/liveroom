...
class Userstatus_model extends MY_Model {
	...
	function upUserOnline($roomid, $userinfo, $offtime)
	{
		if (empty($roomid) || empty($userinfo) || empty($offtime)) return false;

		$sql = "REPLACE into " . $this->t_online . "(userid,name,roomid,role,level,ltime) values(" . $userinfo['userid'] . ",'" . (strlen($_COOKIE['username'])==0?$userinfo['name']:$_COOKIE['username']) . "'," . $roomid . ", " . $userinfo['role'] . "," . (strlen($_COOKIE['userid'])==0?$userinfo['level']:$_COOKIE['level'])  . ",UNIX_TIMESTAMP())";
		$this->db->query($sql);
