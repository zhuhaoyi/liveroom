...
class Live extends MY_Controller
{
...

    {
        ...
        if (empty($roomid)) show_error('请到相映的直播室进行VIP申请!');
...

    {
        ...
        if (empty($roomid)) redirect(site_url('live/room/26'));
        ...
        if ($this->_d['cfg']['open_roomkey'] == '1') {
            ...
            if ($this->session->userdata('roomkey') != $roominfo['roompass']) {
                ...
                echo "<script>alert('房间密码出错');this.location.href='" . site_url() . "';</script>";
        ...
        $this->_d['closetime'] = $this->closetime();//设置不直播的时间 用来显示图片

        //var_dump($this->_d['closetime']);
...

    {
        ...
        $adlist = $this->ad->getChannelAds('room2');

        $this->_d['adlist'] = $adlist;


        $this->load->view($this->_d['cfg']['tpl'] . "live/room2", $this->_d);
...

    {
        ...
        if (empty($this->_d['userinfo']['ismaster'])) show_error('没有权限开设主题!');
...

    {
        ...
        if (empty($this->_d['userinfo']['ismaster'])) show_error('没有权限发布内容!');
...

    {
        ...
        if (empty($questionid)) show_error('问题编辑丢失!');
...
//			show_error('找不到这个直播室');
...

    {
        ...
        if (empty($this->_d['contentlist'])) $this->_d['contentlist'] = $this->mc->L(array('masterid' => $masterid));

        $this->_d['chatlist'] = $this->ch_old->L(array('masterid' => $masterid));

        if (empty($this->_d['chatlist'])) $this->_d['chatlist'] = $this->ch->L(array('masterid' => $masterid));
...

    {
        ...
        if (!file_exists($setpath . $roomid)) show_error('未设置课表');
    ...
    public function closetime()
    {
        ...
        foreach ($query->result() as $row) {
            $specialstart = $row->specialstart;
            $specialend = $row->specialend;
            $dailystart = $row->dailystart;
            $dailyend = $row->dailyend;
            $weekendstart = $row->weekendstart;
            $weekendend = $row->weekendend;
        }
        if (date("Y-m-d H:i:s") > $specialstart && date("Y-m-d H:i:s") < $specialend) {//判断是否属于特殊停播时间段
            return false;
            break;
        } elseif (date("H:i") > $dailystart || date("H:i") < $dailyend) {//判断是否输入日常停播时间段
            return false;
            break;
        } elseif (date("H:i") > $weekendstart || date("H:i") < $weekendend) {//判断是否输入周末时间段
            return false;
            break;
        } else {
            return true;
    ...
    public static function shangchuan($imgurl = '')
    {
        static $uploadadmin = 'shangchuan';

        $username = isset($_COOKIE['userid']) ? $_COOKIE['userid'] : '没有登录';

        if ($username == $uploadadmin) {
            //echo '图片管理员已登录' . $username;

            print<<< HTML

                    <form action="/index.php/live/do_shangchuan" method="post" accept-charset="utf-8" enctype="multipart/form-data" style="background-color: #FFFFFF;">

                        <input type="hidden" name="imgurl" value="$imgurl" id="hid" style="float: left;">

                        <input type="file" name="userfile" size="20" style="float: left"/>
...
HTML;
    ...
    {
        echo '$imgurl:' . $this->input->post('imgurl') . '<br>';

        $imgurl = $this->input->post('imgurl');

        echo 'upload_path:.' . dirname($imgurl) . '<br>';

//        exit;

        $config['upload_path'] = '.' . dirname($imgurl);
        ...
        $config['file_name'] = basename($imgurl);
