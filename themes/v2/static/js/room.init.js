...
function resize(){
	...
	var noticeHeight  = $('.top_notice').height();//房间通知跑马灯高度
	...
	if(windowWidth > WidthLevel.P){	
		...
		$('.topiccontent').height($topic.height() - topicinputHeight -noticeHeight - mmargin*2 -warnmsgHeight -10); //聊天显示块高度
	...
	$('#list_u').height(windowHeight - headerHeight - footerHeight- ulist_above_height - hangqingheight -leftadheight-25); //用户列表高度
...
$(function(){
	...
	$('#sidebar .side1').click(function(){
		TINY.box.show({iframe:'http://baike.baidu.com/link?url=FI_yzPBN9xUqU88fzK5fABJmVt7b7EU5RwoE8EAx1TBAVEh3L7JPNgH6vCK0OU5JZNBG36NDvv4m3MmD_nZQYa',width:1000,height:500,openjs:function(){
			$('.tbox').css('position','absolute');
		}});
	});

	$('#sidebar .side6').click(function(){
		TINY.box.show({iframe:"http://www.wypmo.com/down/download/index",width:1000,height:500,openjs:function(){
			$('.tbox').css('position','absolute');
		}});
	});

	$('#videolink a,#sidebar .side2').click(function(){
