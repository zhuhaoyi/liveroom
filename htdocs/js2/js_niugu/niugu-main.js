...
function telCheck(){
	...
	//$("#applyTel").val(applyTel+1*1);
	var applyTel = $("#applyTel").val();
	if($("#applyName").length>0){
		var uName = $("#applyName").val();	
	}else{
		var uName = '匿名';	
	};
	var tm =/^(?:\d{3})\d{5}(\d{3}|\*{3})$/;
	var tp =/^((\d{2,5})-)?(\d{1,8})(-(\d{3,}))?$/;
	if(!uName){
		alert('请输入用户名');
	}else if(!tm.test(applyTel) && !tp.test(applyTel)){
		//alert("请输入正确的电话号码");
		alertBox('open-wrong');
	}else{
		if(tm.test(applyTel) && applyTel.length<11){
			//alert("请输入正确的电话号码");
			alertBox('open-wrong');
		}else if(tp.test(applyTel) && applyTel.length<7){
			//alert("请输入正确的电话号码");
			alertBox('open-wrong');	
		}else{
			$("#bs").html("正在提交请稍后...").attr("disabled","true");
			var target = $("#bs").attr('url');
			$("form").serializeArray()
			$.get(target,{applyTel:applyTel,uName:uName},function(msg){
				alertBox('open');	
				$("#bs").html("立刻领取油卡").attr("disabled",false);
				//if(msg.info == 'no'){
//					//alert("请输入正确的电话号码");
//					alertBox('open-wrong');	
//					$("#bs").html("立刻领取油卡").attr("disabled",false);
//				}else if(msg.info == 'reg'){
//					$('#open-wrong-content').html('您已经提交过申请了，请耐心等待');
//					alertBox('open-wrong');	
//					$("#bs").html("立刻领取油卡");
//				}else if(msg.info == 'wait'){
//					$('#open-wrong-content').html('系统繁忙，请稍后再试');
//					alertBox('open-wrong');	
//					$("#bs").html("立刻领取油卡").attr("disabled",false);
//				}else{
//					//alert('申请成功');
...
function showLoading(id){
	$('#'+id).html('<div class="loading_div"></div>');	
}