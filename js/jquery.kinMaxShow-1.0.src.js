	...
	$.fn.kinMaxShow = function(user_options){
			...
			var default_options = {
						...
						button:{
								...
								normal:{width:'12px',height:'12px',left:"48%",bottom:'30px',fontSize:'0',background:'#fff',marginRight:'8px',float:'left'},
								//当前 按钮样式
								focus:{background:'#F90'}
							},
			...
			var btnCenter = $("#banner div").length*50/2;
			var btnRight = '-'+btnCenter+'px';
			var cssCode = '<style type="text/css">';
				cssCode+= k.selector+' .'+k.prename+'image_box{width:100%;height:'+parseInt(options.height)+'px;position:relative;z-index:1;} ';
				cssCode+= k.selector+' .'+k.prename+'image_box .'+k.prename+'image_item{width:100%;height:'+parseInt(options.height)+'px;position:absolute;overflow:hidden;} ';
				cssCode+= k.selector+' .'+k.prename+'image_box .'+k.prename+'image_item a{width:100%;height:'+parseInt(options.height)+'px;display:block;text-decoration:none;padding:0;margin:0;background:transparent;text-indent:0;outline:none;hide-focus:expression(this.hideFocus=true);} ';
				if(options.button.normal.display!='none'){
					cssCode+= k.selector+' .'+k.prename+'button{'+k.fn.objToCss(options.button.normal,['top','right','bottom','left'],true)+';margin-left:'+btnRight+';position:absolute;list-style:none;z-index:2;}';
					cssCode+= k.selector+' .'+k.prename+'button li{'+k.fn.objToCss(options.button.normal,['top','right','bottom','left'])+';cursor:pointer;}';
